...
Of course, this is an analysis based on currently available data products. Some combinations of image parameters represent phase shifts into entirely new categories of product - twice-daily revisit time is a fundamentally different capability than continuous coverage, for example. These new capabilities (and the resulting applications imagined in the previous section) could spawn entirely new companies and sources of demand.
...
[^3]: **Note that as of Spring 2024 I have never worked on the Starlink program, nor am I privy to any inside information about the company or their plans going forward. This blog post contains only my speculation based on public information which I've synthesized after following the space for a while.**
